...
export function isWhitelistedComponent(
  ...
  const { includeComponent, ignoreEmptyComponents, excludeComponent } = config;
  ...
  if (includeComponent) {
    ...
    if (typeof includeComponent === "string" && includeComponent.length) {
      return (
        includeComponent.toLowerCase() === componentName?.toLocaleLowerCase()
      );
    }
  ...
  if (excludeComponent) {
    ...
    if (typeof excludeComponent === "string" && excludeComponent.length) {
      return (
        excludeComponent.toLowerCase() !== componentName?.toLocaleLowerCase()
      );
    }
